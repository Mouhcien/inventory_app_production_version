            ...
            <div class="card border-primary mb-3 shadow">
                <div class="card-header">
                    <h5 class="card-title">
                        ...
                        <i class="bi bi-border-style me-1"></i> Les types des entités du DRI-Marrakech
                    </h5>
                </div>
                    ...
                    <table class="table table-light table-striped table-hover shadow">
                        ...
                        @forelse($types as $type)
                            <tr>
                                ...
                                <td> {{ $type->title }}</td>
                                <td>
                                    <x-button-traitement href="{{route('entities.types.edit', $type->id)}}" class="btn btn-sm btn-warning" icon="bi bi-pencil"  />
                                    <x-button-traitement href="{{route('entities.types.show', $type->id)}}" class="btn btn-sm btn-info" icon="bi bi-list-columns"  />
                                    @if(config('app.delete'))
                                        <x-button-modal class="btn btn-sm btn-danger" icon="bi bi-trash" target="deleteModal{{$type->id}}" />
                                        <x-confirmation-modal
                                            href="{{route('entities.types.destroy', $type->id)}}"
                        ...
                        @empty
                            <tr class="align-content-center text-center">
                                <td colspan="3"> <h5> Pas des types d'entité !!!!</h5> </td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
